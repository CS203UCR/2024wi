...
int main(int argc, char **argv)
{
    ...
    if(mode == 0)
    {
        fin = fopen(argv[1], "r");
        for (i = 0; i < (m*p); i++) {
            ret = fscanf(fin,"%lf",&A[i]); 
        }
        fclose(fin);
        fin = fopen(argv[2], "r");
        for (i = 0; i < (p*n); i++) {
            ret = fscanf(fin,"%lf",&B[i]);
        }
        fclose(fin);
    }
    else
    ...
    total_time = ((time_end.tv_sec * 1000000 + time_end.tv_usec) - (time_start.tv_sec * 1000000 + time_start.tv_usec));
     fprintf(stderr,"Data Input Latency: %.0lf us\n",total_time);
    ...
    gettimeofday(&time_end, NULL);
    total_time = ((time_end.tv_sec * 1000000 + time_end.tv_usec) - (time_start.tv_sec * 1000000 + time_start.tv_usec));
    fprintf(stderr,"Latency: %.0lf us, GFLOS: %lf\n",total_time, (double)(ARRAY_SIZE*(double)(ARRAY_SIZE)*(double)ARRAY_SIZE*2/(total_time*1000)));
